...
double getCost(double distance, int mode) {
    ...
    if(mode == 3 || mode == 4) return distance * 7.0;
    return 0.0;
}
...
pair<vector<int>, double> dijkstraFastest(int startNode, int endNode, double startTime) {
    int n = graph.size();
    
    vector<double> minTime(n, INF);
    vector<int> parent(n, -1);
    
    minTime[startNode] = startTime;
    
    priority_queue<pair<double, int>, vector<pair<double, int>>, greater<pair<double, int>>> pq;
    pq.push({startTime, startNode});
    
    while(!pq.empty()) {
        auto [currentTime, id] = pq.top();
        pq.pop();
        
        if(currentTime > minTime[id]) continue;
        if(id == endNode) break;
        ...
        for(auto edge : graph[id]) {
            ...
            double travelTime = (edgeDist / 10.0) * 60.0;
            ...
            if(newArrivalTime < minTime[neighborID]) {
                minTime[neighborID] = newArrivalTime;
                parent[neighborID] = id;
                pq.push({newArrivalTime, neighborID});
            }
        }
    }
...
int main() {
    ...
    pair<vector<int>, double> result = dijkstraFastest(startID, endID, timeAtStartNode);
    vector<int> shortestPath = result.first;
    double arrivalTime = result.second;
    ...
    for(int i = 0; i < shortestPath.size() - 1; i++) {
        ...
        if(edgeMode != currentMode) {
            double travelTime = (segmentDistance / 10.0) * 60.0;
            ...
            if(edgeMode != 1) {
                ...
                if(departure > currentTime) {
                    waitingTime += (departure - currentTime);
