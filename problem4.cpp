...
pair<vector<int>, double> dijkstraCheapest(int startNode, int endNode, double startTime) {
    int n = graph.size();
    ...
    priority_queue<tuple<double, int, double>, vector<tuple<double, int, double>>, greater<tuple<double, int, double>>> pq;
    pq.push({0.0, startNode, startTime});
    
    while(!pq.empty()) {
        auto [currentCost, id, currentTime] = pq.top();
        pq.pop();
        ...
        for(auto edge : graph[id]) {
            ...
            double travelTime = (edgeDist / 30.0) * 60.0;
            ...
            double newCost = minCost[id] + edgeCost;
            ...
            if(newCost < minCost[neighborID] || 
               (newCost == minCost[neighborID] && newArrivalTime < arrivalTime[neighborID])) {
                ...
                pq.push({newCost, neighborID, newArrivalTime});
            }
        }
    }
...
int main() {
    ...
    pair<vector<int>, double> result = dijkstraCheapest(startID, endID, timeAtStartNode);
    ...
    for(int i = 0; i < shortestPath.size() - 1; i++) {
        ...
        if(edgeMode != currentMode) {
            double travelTime = (segmentDistance / 30.0) * 60.0;
